...
本系统通过MQTT协议控制LED矩阵显示屏，支持多设备独立管理和不同分辨率的自动适配。每个设备根据其MAC地址订阅独立的MQTT主题，实现设备级别的消息隔离。
...
**示例：**
- `LED/aabbccdd1122/Text` - 文本显示
- `LED/aabbccdd1122/Clear` - 清屏
- `LED/aabbccdd1122/Brightness` - 亮度设置
- `LED/aabbccdd1122/Image` - 图片显示
...
```
MAC: 246f28a58ec4
MQTT ClientId: ESP32/64x64/246f28a58ec4
MQTT Topics:
  Text: LED/246f28a58ec4/Text
  Clear: LED/246f28a58ec4/Clear
  Brightness: LED/246f28a58ec4/Brightness
...
| line | int | 否 | 1 | 显示行号；滚动模式需在 `1` 到 `最大行数` 之间，静态模式 `line<=0` 表示全屏 |
| wrap | boolean | 否 | true | 静态文本是否自动换行；仅在 `scroll_mode = false` 时有效，滚动模式下忽略 |
...
1. **滚动模式** (`scroll_mode = true`):
   - `line` 必须在 `1` 到 `最大行数` 范围内；`line <= 0` 不会触发全屏显示
   - 不同字体大小的文本可以同时滚动
   - 滚动文本不能全屏显示
   - `wrap` 参数无效，滚动模式下会被忽略
...
2. **静态模式** (`scroll_mode = false`):
   - `line <= 0`（包括-1）时：清屏并全屏显示，清除当前的全屏、静态和滚动内容
   - `line > 0` 时：在指定行显示，保留屏幕其他区域内容；若文本高度跨行，则会清除所占区域内的旧静态和滚动文本
   - `wrap` 仅对静态文本有效；滚动模式下该参数会被忽略
   - `autoWrap = true` 时，DisplayManager 会根据文本长度和字体大小计算实际占用物理行数，并自动换行
...
**示例3：多行滚动文本（64x64设备，MAC为246f28a58ec4）**
...
// MQTT连接配置
const MQTT_SERVER = "zheng241.xyz";
const MQTT_PORT = 1883;
const MQTT_USERNAME = "64x64esp32";
...
**注意**：`wrap` 仅对静态文本有效，滚动模式下该参数会被忽略。前端只有当滚动模式未开启即静态文本时，才显示wrap选项。
