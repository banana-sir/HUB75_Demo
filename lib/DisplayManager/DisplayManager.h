...
public:
    ...
    // ===== 显示配置 =====
    void setTextSize(int size);  // 设置文本大小（仅影响新创建的文本）
    ...
    //
    // 使用说明：
    // - 静态文本时：line<=0（包括-1）清屏并全屏显示，line>0在指定行显示
    // - 滚动文本时：line指定行（1-maxLines）
    // - 每个文本行记录自己的字体大小，不同字体大小的文本可以同时存在
...
private:
    ...
    struct ScrollLine {
        ...
        int updateFrameInterval;  // 更新间隔（帧数，不再是毫秒）
        int frameCounter;     // 帧计数器（累计帧数）
        ...
        ScrollLine() : content(nullptr), xPosition(PANEL_RES_X), yPosition(0),
                       isScrolling(false), isActive(false), updateFrameInterval(2),
                       frameCounter(0), scrollDirection(SCROLL_LEFT), color(0), textSize(1), cachedTextWidth(0) {}
    };
    ...
    // ===== 显示模式标志 =====
    bool isFullScreenDisplay;  // 标记是否为全屏静态文本显示模式（类似图片，不持续渲染）
    char *fullScreenContent;   // 全屏显示的文本内容
    uint16_t fullScreenColor;  // 全屏显示的文本颜色
    bool isImageDisplayMode;   // 标记是否为图片显示模式（不持续渲染）
    bool hasContentToDisplay;   // 标记是否有内容需要显示（仅用于滚动文本和分屏静态文本）
    ...
    // ===== 计算辅助函数 =====
    int calculateScrollInterval(int speed);  // 根据速度等级计算更新间隔（帧数）
    int calculateTextLines(const char *textContent, int startLine, int textSizeParam = 1);  // 计算文本需要占用的行数
    ...
    // ===== 状态管理 =====
    void updateContentDisplayState();  // 更新是否有内容需要显示的状态
