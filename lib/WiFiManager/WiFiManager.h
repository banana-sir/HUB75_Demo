...
private:
    ...
    // ===== 调试相关（条件编译） =====
#if DEBUG_MODE == 1
    unsigned long lastMqttLoopTime;           // 上次输出MQTT心跳的时间
    int lastMqttState;                      // 上次MQTT状态
#endif
    ...
    // ===== 辅助私有方法 =====
    void displayStatusMessage(const char* message);  // 显示状态消息到LED屏幕
    void subscribeToMqttTopics();               // 订阅所有MQTT主题
