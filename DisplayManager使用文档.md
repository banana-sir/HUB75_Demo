...
unsigned long frameInterval;  // 目标帧间隔（毫秒），默认16ms = 60fps
```
...
frameInterval = 16ms  →  60fps  (推荐)
frameInterval = 33ms  →  30fps
frameInterval = 12ms  →  83fps
```
